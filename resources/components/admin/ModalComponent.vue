      ...
      <div class="fixed inset-0 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4 text-center" >
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95" >
            <DialogPanel class=" w-full max-w-xl transform overflow-hidden rounded-2xl bg-white dark:bg-zinc-600 p-6 text-left align-middle shadow-xl transition-all " >
              ...
              <div class="mt-2">
                <KeepAlive>
                  <component :is="this.form" :idProp="this.idProp" :resource="resource" @closeModal=" isClose = !false; $emit('closeModal'); "
                   @closeAndUpdate="$emit('update'); $emit('closeModal');isClose = !false;" />
                </KeepAlive>
              </div>
              <div class="absolute top-6 right-6">
                <button type="button" class="" @click="(isClose = !isClose), $emit('closeModal')" >
